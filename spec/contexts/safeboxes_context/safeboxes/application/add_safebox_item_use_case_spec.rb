...
RSpec.describe SafeboxesContext::Safeboxes::Application::AddSafeboxItemUseCase, type: :use_case do
  describe "#create(input:)" do
    ...
    context "with a locked safebox" do
      it "raises an exception" do
        input = instance_double(SafeboxesContext::Safeboxes::Infrastructure::AddSafeboxItemInput, safebox_id:)

        allow(repository).to receive(:find_by_id).with(safebox_id) do
          SafeboxesContext::Safeboxes::Domain::SafeboxEntityFactory.build(:locked, id: safebox_id)
        end
    ...
    context "with an invalid token" do
      it "raises an exception" do
        input = instance_double(SafeboxesContext::Safeboxes::Infrastructure::AddSafeboxItemInput, safebox_id:, token:)

        allow(repository).to receive(:find_by_id).with(safebox_id) do
          SafeboxesContext::Safeboxes::Domain::SafeboxEntityFactory.build(id: safebox_id)
        end
        ...
        expect do
          described_class.new(repository:).create(input:)
        end.to raise_error(SafeboxesContext::Safeboxes::Domain::Errors::InvalidSafeboxTokenError)
      end
    end
    ...
    context "with invalid attributes" do
      it "does not return any safebox item" do
        input = instance_double(
          SafeboxesContext::Safeboxes::Infrastructure::AddSafeboxItemInput,
          invalid?: true,
          errors: ["<error>"],
          data: attributes,
          safebox_id:,
    ...
    context "with valid attributes" do
      it "adds new item to the safebox" do
        ...
        safebox = SafeboxesContext::Safeboxes::Domain::SafeboxEntityFactory.build(id: safebox_id)
        safebox_item = SafeboxesContext::Safeboxes::Domain::SafeboxItemEntityFactory.build(
          id: input.data[:id],
      ...
      it "returns safebox" do
        ...
        expect(result.safebox_item).to be_a(SafeboxesContext::Safeboxes::Domain::SafeboxItemEntity)
      end
