...
module Api
  class ApplicationController < ActionController::API
    ...
    def failed_response(errors, status: :bad_request)
      errors = Error.generate_from(errors)

      render json: { errors: }, status:
    end

    rescue_from StandardError, ScriptError do |e|
      error = InternalServerError.new(e)

      failed_response(error, status: :internal_server_error)
    end

    rescue_from NotAcceptableMediaTypeError do |e|
      failed_response(e, status: :not_acceptable)
    end

    rescue_from UnsupportedMediaTypeError do |e|
      failed_response(e, status: :unsupported_media_type)
    end

    rescue_from ActiveRecord::RecordInvalid do |e|
      duplicated_record_errors, invalid_argument_errors = e.record.errors.partition do |error|
        error.attribute == :id && error.type == :taken
      end

      if duplicated_record_errors.any?
        error = DuplicatedRecordError.new(e.record.id, e.record.class.name)

        failed_response(error, status: :conflict)
      else
        errors = InvalidArgumentError.generate_from(invalid_argument_errors)
    ...
    rescue_from ActiveRecord::RecordNotFound do |e|
      error = RecordNotFoundError.new(e.id, e.model)

      failed_response(error, status: :not_found)
    end

    rescue_from LockedSafeboxError do |e|
      failed_response(e, status: :locked)
    end

    rescue_from InvalidSafeboxPasswordError do |e|
      e.header = OPEN_SAFEBOX_HEADER

      failed_response(e, status: :unauthorized)
    end

    rescue_from InvalidSafeboxTokenError do |e|
    ...
    def check_accept_header
      ...
      raise NotAcceptableMediaTypeError.new(
    ...
    def check_content_type_header
      ...
      raise UnsupportedMediaTypeError.new(
